...
import {TakeAsk, TakeBid, TakeAskSingle, TakeBidSingle, Order, Exchange, Fees, FeeRate, AssetType, OrderType, Transfer, FungibleTransfers, StateUpdate, AtomicExecution, Cancel, Listing} from "../src/struct/Structs.sol";
...
contract XYDExchangeV2Test is Test {
    ...
    function setUp() public {
        ...
        console.log(
            unicode"实现合约 XYDManagerV2 部署地址:",
            address(xydExchangeV2)
        );
    ...
    function test_takeAskSingle_Success() public {
        ...
        uint256 takerFeeAmount = (TEST_PRICE *
            testTakeAskSingle.takerFee.rate) / 10000; // 1% 手续费
        uint256 makerFeeAmount = (TEST_PRICE *
            testTakeAskSingle.order.makerFee.rate) / 10000; // 0.5% 卖家手续费
        uint256 expectedEthSpent = TEST_PRICE + takerFeeAmount + makerFeeAmount;
        assertEq(
            beforeBuyerEth - buyer.balance,
            expectedEthSpent,
            unicode"买家 ETH 消耗错误"
        );
    ...
    function _hashFeeRate(
        ...
        return
            keccak256(
                abi.encode(_FEE_TYPEHASH, feeRate.recipient, feeRate.rate)
            );
    }
    ...
    function _hashToSign(
        ...
        bytes32 signatures = keccak256(
            bytes.concat(bytes2(0x1901), _DOMAIN_SEPARATOR, orderhash)
        );
